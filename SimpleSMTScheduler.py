...
if __name__ == "__main__":
    ...
    if interactive:
        tasksFileName = 'examples/demo_tasks.csv'#input("Enter the csv file for the tasks to be scheduled: ")
        wcet_offset = 0#int(input("Enter the WCET allocation gap between activations: "))
        schedulePlotPeriods = 2#int(input("Enter the number of hyper periods to be plotted: "))
        code = 'yes'#input("Enable code output (Yes/No)? ").lower() == "Yes".lower()
        optimize = 'yes'#input("Enable optimization (Yes/No)? ").lower() == "Yes".lower()
        verbose = 'yes'#input("Enable verbose mode (Yes/No)? ").lower() == "Yes".lower()
